function wszystkie_kolory_new(img)
...
% Wyświetl oryginalny obraz i obraz binarny
% figure(1);
% subplot(2, 1, 1);
% imshow(img);
...
% Wyświetlenie obrazu po wykonaniu erozji
% figure(2);
% imshow(erodedImage);
% title('Obraz po operacji erozji');


% Wyświetlenie obrazu po wykonaniu dylatacji
% figure(3);
% imshow(dilatedImage);
% title('Obraz po operacji dylatacji');


% Oblicz minimalny rozmiar regionu (np. 1% powierzchni obrazu)
percent_of_image_min1 = 0.05; % Dostosuj tę wartość według potrzeb
min_region_size1 = calculate_region_size(height, width, percent_of_image_min1);


percent_of_image_min2 = 0.1; % Dostosuj tę wartość według potrzeb
...
coefficient = 0.04; % Dostosuj według potrzeb
some_threshold = calculate_relative_threshold(img, coefficient);



detected_regions=detect_regions(dilatedImage, min_region_size1, some_threshold, max_region_size, min_region_size2);


display_detected_regions(img, detected_regions);

display_detected_regions(dilatedImage, detected_regions);


% display_detected_regions(binary_img, detected_regions);
